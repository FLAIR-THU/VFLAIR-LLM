...
- **GPT2ModelTail** receives the rest of the encoders from full_gpt in its \_\_init_\_() function. Then in its forward() function, it receives intermediate results from local model and further iterate through its own encoder layers and final linear layer to produce the global output. 
...
class GPT2ModelTail(GPT2ModelSplitter):
    ...
    def forward(
        ...
        **kwargs
        ...
        self.base_model_tail_output = hidden_states # store pooled output for snd defense
...
> Wrap  `class GPT2ModelTail`  into downstream model classes based on type of task needed.
...
The transformer package provide  `class GPT2ForSequenceClassification` which consists of a complet GPT2 model('class GPT2Model') and a linear classifier as head layer. In this step we shall wrap the former `class GPT2ModelTail`  into `class GPT2TailForSequenceClassification` . 

- **GPT2TailForSequenceClassification** build upon the (`class GPT2ModelTail`)

```python
class GPT2TailForSequenceClassification(GPT2ForSequenceClassification, VFLModel):
