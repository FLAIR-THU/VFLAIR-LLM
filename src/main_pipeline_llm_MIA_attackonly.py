...
def evaluate_no_attack_pretrained(args):
    ...
    exp_result, metric_val = vfl.inference(need_save_state = args.need_final_epoch_state, attack_only = 1)

    # # Save record 
    exp_result = "NoAttack|{args.pad_info}|seed={args.current_seed}|K={args.k}" + exp_result
    print(exp_result)
...
def create_exp_dir_and_file(dataset, vfl_model_slice_num, split_info, model_name, pipeline, defense_name='', defense_param=''):
    exp_res_dir = f'exp_result/MIA_attackonly/{dataset}/{str(vfl_model_slice_num)}-slice/{split_info}/'
...
if __name__ == '__main__':
    ...
    print('attack_only:',args.attack_only)
