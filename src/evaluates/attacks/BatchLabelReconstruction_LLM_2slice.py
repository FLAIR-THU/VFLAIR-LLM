...
class BatchLabelReconstruction_LLM_2slice(Attacker):
    ...
    def attack(self):
        ...
        for attacker_ik in self.party: # attacker party #attacker_ik
            ...
            for iters in range(1, self.epochs + 1):
                ...
                dummy_onehot_label = dummy_label #F.softmax(dummy_label, dim=-1)
                dummy_loss = self.cal_loss(active_model_body_pred, dummy_onehot_label)
                
                # dummy_gradient = torch.autograd.grad(dummy_loss, dummy_onehot_label, create_graph=True, allow_unused=True)[0]
                # print(f'dummy_gradient:{type(dummy_gradient)} {dummy_gradient.shape}')
                ...
                # loss: L-L'
                grad_diff = 0
                # grad_diff = cross_entropy_for_onehot(dummy_gradient, global_gradient)
