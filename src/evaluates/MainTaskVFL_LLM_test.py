        ...
        def save_state(self, BEFORE_MODEL_UPDATE=True):
            if self.args.need_save_state:
                if BEFORE_MODEL_UPDATE:
                    # local_model_head_path = self.args.exp_res_dir + 'self.parties[0].local_model'
                    # print('local_model_head_path:',local_model_head_path)

                    # _f = 0
                    # for name, param in self.parties[0].local_model.named_parameters():
                    #     print(f'{name} {param[0]}')
                    #     _f += 1
                    #     if _f >3:
                    #         break

                    # self.parties[0].local_model.save_pretrained(local_model_head_path)
                    # new_local_model_head = self.parties[0].local_model._load_model_head(local_model_head_path,do_split=False,**self.args.kwargs_model_loading)

                    # _f = 0
                    # for name, param in new_local_model_head.named_parameters():
                    #     print(f'{name} {param[0]}')
                    #     _f += 1
                    #     if _f >3:
                    #         break
