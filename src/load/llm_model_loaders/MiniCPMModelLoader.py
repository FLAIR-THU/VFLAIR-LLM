...
from models.llm_models.minicpm import ModelPartitionPipelineMiniCPM
...
class MiniCPMModelLoader(LLMModelLoader):
    ...
    def load(self, args, model_path, is_active_party):
        ...
        if args.model_type == 'MiniCPMV':
            if args.vfl_model_slice_num == 3:
                for _key in self._models.keys():
                    if _key == 0:
                        self._models[_key] = MiniCPMVModelHead(self._models[_key].to(args.device)).to(args.device)
                    elif _key == 1:
                        self._models[_key] = MiniCPMVModelBody(self._models[_key].to(args.device)).to(args.device)
                    elif _key == 2:
