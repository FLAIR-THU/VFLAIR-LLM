...
class MessageService:
    ...
    def parse_message(self, message):
        if message.type == fpm.LOAD_MODEL:
            ...
            config = message.data.named_values['config']
            return self._task_service.load_model(model_type.string, model_id.string, config.string)
            ...
            return self.start_task(task_value.string, data_value)
        elif message.type == fpm.UPDATE_MODEL_DATA:
            job_id = message.data.named_values['job_id']
            return self._task_service.update_model_data(job_id.sint64)
        elif message.type == fpm.UPDATE_CONFIG:
            job_id = message.data.named_values['job_id']
            config_value = message.data.named_values['config']
            return self._task_service.update_config(job_id.sint64, config_value.string)

    def start_task(self, task_value, data_value):
        ...
        result = self._task_service.run_specific(task, data=data)
        value = fpm.Value()
        if result is None:
            return None
