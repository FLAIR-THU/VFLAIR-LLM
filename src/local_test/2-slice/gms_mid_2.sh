...
for seed in 1 2 3
    ...
    # 0.01
    sed -i 's/"lambda": 0.1/"lambda": 0.01/g' ./configs/2-slice/gms_mia_mid.json
    python main_pipeline_llm_MIA.py --seed $seed --configs 2-slice/gms_mia_mid

    # 0.001
    sed -i 's/"lambda": 0.01/"lambda": 0.001/g' ./configs/2-slice/gms_mia_mid.json
    python main_pipeline_llm_MIA.py --seed $seed --configs 2-slice/gms_mia_mid

    # 0.0001
    sed -i 's/"lambda": 0.001/"lambda": 0.0001/g' ./configs/2-slice/gms_mia_mid.json
    python main_pipeline_llm_MIA.py --seed $seed --configs 2-slice/gms_mia_mid

    # 1e-5
    sed -i 's/"lambda": 0.0001/"lambda": 0.00001/g' ./configs/2-slice/gms_mia_mid.json
    python main_pipeline_llm_MIA.py --seed $seed --configs 2-slice/gms_mia_mid
