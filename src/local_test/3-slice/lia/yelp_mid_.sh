#!/bin/bash
#SBATCH --job-name yelp_lia_mid_           # 任务名叫 example
#SBATCH --gres gpu:a100:2                # 每个子任务都用一张 A100 GPU
#SBATCH --time 1-5:00:00                    # 子任务 1 天 1 小时就能跑完
#SBATCH --output exp_result/yelp_lia_mid_.out

for seed in 60 61 62 63 64 65
    do
    # 0.5
    # python main_pipeline_llm.py --prefix "lia" --seed $seed --configs 3-slice/lia/yelp_mid_
    ...
    # 1e-5
    sed -i 's/"lambda": 0.0001/"lambda": 1e-5/g' ./configs/3-slice/lia/yelp_mid_.json
    python main_pipeline_llm.py --prefix "lia" --seed $seed --configs 3-slice/lia/yelp_mid_

    sed -i 's/"lambda": 1e-5/"lambda": 0.5/g' ./configs/3-slice/lia/yelp_mid_.json
