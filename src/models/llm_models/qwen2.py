...
class Qwen2ModelSplitter(Qwen2Model, VFLModel):
    ...
    def _split_layers(self, idx_of_layers: Iterable[int]) -> bool:
        # print(f'Qwen2ModelSplitter _split_layers {list(idx_of_layers)}')
...
class Qwen2ModelBody(Qwen2ModelSplitter):
    ...
    def forward(
        ...
        ## no need to prepare attention_mask again
        # if attention_mask is not None and self._attn_implementation == "flash_attention_2" and use_cache:
        #     is_padding_right = attention_mask[:, -1].sum().item() != batch_size
        #     if is_padding_right:
        #         raise ValueError(
...
class Qwen2TailForSequenceClassification(Qwen2ForSequenceClassification, VFLModel):
    def __init__(self, config: Qwen2Config, **kwargs):
        super().__init__(config)
        self.model = Qwen2ModelTail(config)
        # Initialize weights and apply final processing
...
class ModelPartitionPipelineQwen2(ModelPartitionPipeline):
    ...
    def _load_model_tail(self, model_name_or_path, do_split=False, **kwargs) -> Union[PreTrainedModel, VFLModel]:
        if self.args.model_architect == 'CLM':
            ...
            model_tail = Qwen2TailForSequenceClassification.from_pretrained(model_name_or_path, **kwargs)
        # elif self.args.model_architect == 'TQA':
        #     model_tail = Qwen2TailForQuestionAnswering.from_pretrained(model_name_or_path, **kwargs)
        else:
