...
class BaichuanModelSplitter(BaichuanModel, VFLModel):
    ...
    def _split_layers(self, idx_of_layers: Iterable[int]) -> bool:
        # print(f'BaichuanModelSplitter _split_layers {list(idx_of_layers)}')
...
class BaichuanModelHead(BaichuanModelSplitter):
...
class BaichuanModelBody(BaichuanModelSplitter):
    def __init__(self, config: BaiChuanConfig):
        super().__init__(config)
        self.past_key_values = None
        del self.norm
...
class BaichuanModelTail(BaichuanModelSplitter):
    def __init__(self, config: BaiChuanConfig):
        super().__init__(config)
        self.past_key_values = None
        # del self.embed_tokens
...
class BaichuanTailForCausalLM(BaiChuanForCausalLM, VFLModel):
    def __init__(self, config: BaiChuanConfig, **kwargs):
        super().__init__(config)
        self.model = BaichuanModelTail(config)
...
class ModelPartitionPipelineBaichuan(ModelPartitionPipeline):

    def _load_model_head(self, model_name_or_path, do_split=False, **kwargs) -> Union[PreTrainedModel, VFLModel]:
        model_head = BaichuanModelHead.from_pretrained(model_name_or_path, **kwargs)
        if do_split:
    ...
    def _load_model_tail(self, model_name_or_path, do_split=False, **kwargs) -> Union[PreTrainedModel, VFLModel]:
        if self.args.model_architect == 'CLM':
            model_tail = BaichuanTailForCausalLM.from_pretrained(model_name_or_path, **kwargs)
        # elif self.args.model_architect == 'CLS':
        #     model_tail = BaichuanTailForSequenceClassification.from_pretrained(model_name_or_path, **kwargs)
        # elif self.args.model_architect == 'TQA':
        #     model_tail = BaichuanTailForQuestionAnswering.from_pretrained(model_name_or_path, **kwargs)
        else:
    ...
    def _load_model_body(self, model_name_or_path, do_split=False, **kwargs) -> Union[PreTrainedModel, VFLModel]:
        model_body = BaichuanModelBody.from_pretrained(model_name_or_path, **kwargs)
        if do_split:
