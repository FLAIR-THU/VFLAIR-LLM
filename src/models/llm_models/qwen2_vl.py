...
class Qwen2VLModelSplitter(Qwen2VLModel, VFLModel):
...
class Qwen2VLModelHead(Qwen2VLModelSplitter):
...
class Qwen2VLModelBody(Qwen2VLModelSplitter):
    def __init__(self, config: Qwen2VLConfig):
        super().__init__(config)
        self.past_key_values = None
        del self.norm
    ...
    def forward(
        ...
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        position_embeddings: Optional[torch.Tensor] = None, # add
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        use_cache: Optional[bool] = None,
        ...
        **kwargs
        ...
        # receive position_embeddings from input kwargs
        # create position embeddings to be shared across the decoder layers
        # position_embeddings = self.rotary_emb(hidden_states, position_ids)
...
class Qwen2VLModelTail(Qwen2VLModelSplitter):
    def __init__(self, config: Qwen2VLConfig):
        super().__init__(config)
        self.past_key_values = None
...
# Model Head Wrapper
class Qwen2VLHeadForConditionalGeneration(Qwen2VLForConditionalGeneration, VFLModel):
    def __init__(self, config):
        super().__init__(config)
        self.visual = Qwen2VisionTransformerPretrainedModel._from_config(config.vision_config)
        self.model = Qwen2VLModelHead(config)
...
# Model Tail Wrapper
class Qwen2VLTailForConditionalGeneration(Qwen2VLForConditionalGeneration, VFLModel):
    def __init__(self, config):
        super().__init__(config)
        self.visual = Qwen2VisionTransformerPretrainedModel._from_config(config.vision_config)
        self.model = Qwen2VLModelTail(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self.padding_side = "left"  # set it to left by default, user can use setter to change padding_sides
    ...
    def forward(
        ...
        **kwargs
        ...
        outputs = self.model(
            ...
            past_key_values=past_key_values,
            inputs_embeds=inputs_embeds,
            position_embeddings = position_embeddings, # add
...
class ModelPartitionPipelineQwen2VL(ModelPartitionPipeline):
    ...
    def _load_model_head(self, model_name_or_path, do_split=False, **kwargs) -> Union[PreTrainedModel, VFLModel]:
        if self.args.model_architect == 'CLM':
            model_head = Qwen2VLHeadForConditionalGeneration.from_pretrained(model_name_or_path, **kwargs)
        else:
    ...
    def _load_model_tail(self, model_name_or_path, do_split=False, **kwargs) -> Union[PreTrainedModel, VFLModel]:
        if self.args.model_architect == 'CLM':
            model_tail = Qwen2VLTailForConditionalGeneration.from_pretrained(model_name_or_path, **kwargs)
        else:
            raise ValueError(f"model_architect {self.args.model_architect} not supported for {model_name_or_path}")
    ...
    def _load_model_body(self, model_name_or_path, do_split=False, **kwargs) -> Union[PreTrainedModel, VFLModel]:
        model_body = Qwen2VLModelBody.from_pretrained(model_name_or_path, **kwargs)
        if do_split:
