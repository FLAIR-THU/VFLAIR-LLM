...
from .conversation import StoppingCriteriaSub # minigpt4.conversation.conversation

class MiniGPTBaseHead(BaseModel):
    """
    ...
    def config(self):
        ...
        # else:
        return self.llama_model.config
    ...
    def config(self, config):
        if config is None:
            ...
            self.llama_model.config = config

    def _clear_past_key_values(self):
        self.llama_model._clear_past_key_values()
    ...
    def preparing_embedding(self, samples):
        ...
        if 'conv_q' in samples:
            ...
            # Done in party_dataset.py
            # self.llama_tokenizer.padding_side = "right"
            # text = [t + self.end_sym for t in samples["answer"]]
            # # print('final answer text:',text)
    ...
    def align_data_input(self, samples, labels, **kwargs):
        '''
        Preprocess image+text --> inputs_embeds
        '''
        # prepare the embedding to condition and the embedding to regress
        cond_embeds, cond_atts= \
            self.preparing_embedding(samples) # , regress_embeds, regress_atts, part_targets 
        # print('cond_embeds:',cond_embeds.shape, cond_embeds[0,:3,:3]) # 1 86 4096
        ...
        # print('align_data_input--kwargs:',type(kwargs),kwargs.keys())
    ...
    def forward(self, samples = None, reduction='mean',
        ...
        if samples != None:
            ...
            outputs = self.llama_model(**kwargs)
...
class MiniGPTBaseBody(BaseModel):
    """
    Base class for MiniGPT-4 and MiniGPT-v2
    """
    ...
    def forward(self, inputs_embeds = None, attention_mask = None, reduction = 'mean', **kwargs):
        ...
        with self.maybe_autocast():
            outputs = self.llama_model(
                inputs_embeds, attention_mask, reduction,return_dict=True, **kwargs
            )
            # outputs = self.llama_model(
            #     inputs_embeds=inputs_embeds,
            #     attention_mask=attention_mask,
...
class MiniGPTBaseTail(BaseModel):
    """
    Base class for MiniGPT-4 and MiniGPT-v2
    """
    ...
    def prepare_inputs_for_generation(
        ...
        return self.llama_model.prepare_inputs_for_generation(input_ids, past_key_values, attention_mask, inputs_embeds, cache_position, **kwargs)

    # def prepare_inputs_for_generation(self, **kwargs):
    #     return self.llama_model.prepare_inputs_for_generation(**kwargs)